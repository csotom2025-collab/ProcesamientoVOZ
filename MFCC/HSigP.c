...
void PreEmphasise (Vector s, float k)
{
   ...
   /*for (i=1;i<=130;i++)
      printf ("El valor de la muestra s[%d]=%f",i,s[i]);*/
      
   for (i=VectorSize(s);i>=2;i--)
      s[i] -= s[i-1]*preE;
      
    for (i=1;i<=30;i++)
      printf ("\n \n El valor de la muestra s[%d]=%f\n",i,s[i]);
                    ...
                    float alpha, float warpLowCut, float warpUpCut)
{
  ...
  /* printf ("El valor de mhi es = %f\n",mhi);
   printf ("El valor de mlo es = %f\n",mlo);
   printf ("El valor de fb.res es = %f\n",fb.fres);*/

   printf ("El valor de fb.klo es = %d\n",fb.klo);
   printf ("El valor de fb.khi es = %d\n",fb.khi);
      printf ("El valor de fb.klo es = %d\n",fb.klo);
   ...
   for (chan=1; chan <= maxChan; chan++) {
	...
	/* printf ("el valor de fb.cf[%d] es = %f\n",chan,fb.cf[chan]);*/
      }
      else {
   ...
   for (k=1,chan=1; k<=Nby2; k++){
      ...
      if (k<fb.klo || k>fb.khi) fb.loChan[k]=-1;
         ...
         fb.loChan[k] = chan-1;
      	 ...
      	 /*printf ("el valor de fb.loChan[%d] es = %d\n",k,fb.loChan[k]);  */
   ...
   for (k=1; k<=Nby2; k++) {
      ...
      if (k<fb.klo || k>fb.khi) fb.loWt[k]=0.0;
         ...
         if (chan>0) {
            /* printf ("el valor de fb.cf_chan[%d] es = %f\n",chan,fb.cf[chan]);
             printf ("el valor de fb.cf_chan+1[%d] es = %f\n",chan+1,fb.cf[chan+1]);
             printf ("el valor de Mel[k] es = %f\n",Mel(k,fb.fres));
             */
     ...
     /*printf ("el valor de fb.loWt[%d] es = %f\n",k,fb.loWt[k]); determina la porciòn de la frecuencia del lado izquierdo por canal*/
   }
...
void Wave2FBank(Vector s, Vector fbank, float *te, FBankInfo info)
{
  ...
  /* for (k=1; k<=info.frameSize; k++) 
   ...
   /*for (k=1; k<=info.frameSize; k++) 
      printf ("el valor de info.x[%d] es = %f\n",k,info.x[k]);  */
   ...
   /*printf ("el valor de info.hlo es = %d\n",info.klo); info.klo=2
   printf ("el valor de info.khi es = %d\n",info.khi); info.khi=128*/
   
   for (k = info.klo; k <= info.khi; k++) {             /* fill bins */
   
      t1 = info.x[2*k-1]; t2 = info.x[2*k];
      
      /*printf ("el valor de ek=%d\n",info.usePower); usePower=0*/
      if (info.usePower)
         ek = t1*t1 + t2*t2;
      else
         ek = sqrt(t1*t1 + t2*t2);
      bin = info.loChan[k];
      /*printf ("el valor de k=%d de bin es  %d\n",k,bin);
      printf ("el valor de info.loWt[%d]=%f\n",k,info.loWt[k]);*/
      
      /*printf ("el valor de ek=%f\n",ek); hasta aquì todo bien*/
      t1 = info.loWt[k]*ek;
      if (bin>0) fbank[bin] += t1;
      if (bin<info.numChans) fbank[bin+1] += ek - t1;
      
      /*printf ("el valor de fbank[%d]=%f \n",bin,fbank[bin]);*/
   }

	/*printf ("el valor de info.takeLogs=%d \n",info.takeLogs); takeLogs=1 */
	/*printf ("el valor de melfloor=%f \n",melfloor); */
   /* Take logs */
   if (info.takeLogs)
...
void FBank2MFCC(Vector fbank, Vector c, int n)
{
   ...
   printf ("el valor de numChan=%d\n",numChan);*/
   
   /*printf ("el valor de mfnorm=%f \n",mfnorm);
   printf ("el valor de pi_factor=%f\n",pi_factor);*/
   ...
   for (j=1; j<=n; j++)  {
	 ...
	 /*printf ("el valor de cos=%d %f\n",j,cos(x*(k-0.5)));*/
      
      }
      c[j] *= mfnorm;
      /*printf ("el valor de c[%d]=%f \n",j,c[j]);*/
   }        
}
...
void MelSpec2FBank(Vector melspec)
{
   ...
   for (i=1; i<=VectorSize(melspec); i++){
      ...
      if (x<1.0) x = 1.0;
      melspec[i] = log(x);
      /*printf ("el valor de melspec[%d]=%f \n",i,melspec[i]);*/
   }
}
                    ...
                    int delwin, int head, int tail, Boolean simpleDiffs)
{
   ...
   printf ("El valor de n = %d\n",n);
   printf ("El valor de step = %d\n",step);
   printf ("El valor de offset = %d\n",offset);
   printf ("El valor de delwin = %d\n",delwin);
   printf ("El valor de vSize = %d\n",vSize);
   printf ("El valor de simpleDiffs = %d\n",simpleDiffs);
